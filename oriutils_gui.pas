...
procedure MessageDlg(const S: String); overload;
...
function OpenFileDialog(var Params: TFileDialogParams): Boolean; overload;
function SaveFileDialog(var Params: TFileDialogParams): Boolean; overload;
function OpenFileDialog(const Title, Filters, Ext: String; var FileName: String): Boolean; overload;
function SaveFileDialog(const Title, Filters, Ext: String; var FileName: String): Boolean; overload;
function FileDlgParams(const Title, Filters, Ext: String): TFileDialogParams;
{%endregion}

function GetMaxTopChild(AParent: TWinControl): Integer;
...
begin
  ...
  Result.DefaultExt := Ext;
  Result.MultiSelect := False;
  Result.FileNames := nil;
end;

function OpenFileDialog(const Title, Filters, Ext: String; var FileName: String): Boolean;
var Params: TFileDialogParams;
begin
  Params := FileDlgParams(Title, Filters, Ext);
  Params.FileName := FileName;
  Result := OpenFileDialog(Params);
  if Result then FileName := Params.FileName;
end;

function SaveFileDialog(const Title, Filters, Ext: String; var FileName: String): Boolean;
var Params: TFileDialogParams;
begin
  Params := FileDlgParams(Title, Filters, Ext);
  Params.FileName := FileName;
  Result := SaveFileDialog(Params);
  if Result then FileName := Params.FileName;
end;
...
begin
  ...
  begin
    ...
    lpstrFilter := PChar(StringReplace(Params.Filters, '|', #0, [rfReplaceAll]) + #0#0);
...
function SaveFileDialog(var Params: TFileDialogParams): Boolean;
begin
  {$ifdef WINDOWS}
    Result := WinApiFileDialog(Params, wadtSave);
  {$else}
    Result := False; // TODO
  {$endif}
end;
...
// Можно использовать при размещении контролов друг под другом. Если добавить
// новый контрол и просто выставить Align = alTop, то он помещается над старыми.
// Нужно выставить ему нужный Top, чтобы он встал в самый низ (обычно еще +1).
function GetMaxTopChild(AParent: TWinControl): Integer;
var i: Integer;
begin
  Result := -1;
  for I := 0 to AParent.ControlCount-1 do
    with AParent.Controls[I] do if Top > Result then Result := Top + Height;
end;
end.
